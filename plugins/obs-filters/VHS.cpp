...
static obs_properties_t *mask_filter_properties(void *data)
{
	...
	obs_property_list_add_string(p,
		obs_module_text("MaskBlendType.Custom"),
		"custom.effect");
...
static void mask_filter_render(void *data, gs_effect_t *effect)
{
	...
	param = gs_effect_get_param_by_name(filter->effect, "rand");
	gs_effect_set_float(param, filter->rand);
	obs_source_process_filter_end(filter->context, filter->effect, 0, 0);

	UNUSED_PARAMETER(effect);
}

struct obs_source_info VHS = {
	.id = "VHS",
